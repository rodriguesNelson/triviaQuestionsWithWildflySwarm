package com.atsistemas.njar.tester;
                 ...
                 // The set method has 1 argument, which is of the same type as the return type of the get method, so we can test this property
                    try{
                        ...
                        if (value instanceof Date) {
                        	final Object clonedValue = ((Date) value).clone();
                            assertEquals(String.format("Failed while testing property %s", prop.getName()), actualValue, clonedValue);
                            assertNotSame(String.format("Failed while testing property %s", prop.getName()), actualValue, clonedValue);
                        }
                    } catch (Exception ex){
