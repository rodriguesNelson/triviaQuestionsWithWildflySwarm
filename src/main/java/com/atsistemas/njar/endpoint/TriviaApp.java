...
public class TriviaApp extends Application {
	...
	public Response getDirectory(@Context UriInfo uri) {
		Link seftLink = Link.fromUri(uri.getBaseUri()).rel("self").type(MediaType.APPLICATION_JSON).build();
		Link questionsLink = Link.fromUri(uri.getBaseUri() + "questions").rel("questions")
				.type(MediaType.APPLICATION_JSON).build();
		return Response.ok().lastModified(new Date()).location(uri.getRequestUri()).links(seftLink, questionsLink)
				.build();
	}
}
